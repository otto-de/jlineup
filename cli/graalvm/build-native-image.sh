...
`#--initialize-at-build-time=tools.jackson,javassist.ClassPool` \
--verbose \
--report-unsupported-elements-at-runtime \
`#--static` \
`#-H:+TraceSecurityServices` \
`#-H:+TraceClassInitialization` \
-jar build/libs/jlineup-cli-4.15.0-all.jar
...
mv jlineup-cli-4.15.0-all build/libs/jlineup-cli-4.15.0-all
rm ~/.m2/repository/webdriver -rf
./build/libs/jlineup-cli-4.15.0-all -Dwdm.architecture=X64 --config graalvm/lineup_chrome_headless.json --step before

set +e

./build/libs/jlineup-cli-4.15.0-all -Dwdm.architecture=X64 --config graalvm/lineup_chrome_headless.json --step after
